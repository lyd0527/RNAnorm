...
/*void FileOperate::GenToFPKM(string srcFile, string dstFile)
{
	...
	while (getline(inFile, str))
	{
		...
		vector<string> tmp;  // vectorÓÃÀ´´æ·ÅÃ¿ÐÐÊý¾Ý
		string a;
		while (input >> a)
		{
			tmp.push_back(a);
	...
	cout<<"num[1]="<<num[1].size()<<endl;
	cout<<"num.size="<<num.size()<<endl;	
	float sum[21] = { 0 }; // sumÊý×éÓÃÀ´´æ·ÅÁÐºÍ  ±ãÓÚºóÐøFPKM¼ÆËã
	for (int j = 0; j < sizeof(sum) / sizeof(sum[0]); j++)
	{
		for (int i = 1; i < num.size(); i++)
		{
			float aa =(float)atof( num[i][j + 2].c_str());  // ½«vector¶Áµ½µÄÊý¾Ý×ªÎªfloatÐÍ
			sum[j] += aa;
		}
	}

	// ¶Ô³ý±íÍ·ÒÔÍâµÄÊý¾Ý½øÐÐ²Ù×÷
	for (int i = 1; i < num.size(); i++)
	{
...
/*void FileOperate::DataFilter(string srcFile, string dstFile, string op,float val)
{
	...
	float median[49] = { 0 };  // ´æ·ÅÔ´Êý¾ÝÖÐÎ»Êý¼°×îºóËùÇó¾ø¶ÔÖÐÎ»²îµÄÖÐÎ»Êý
	////CVÖµ
	float cv[49] = { 0 };
	float avg[49] = { 0 };  // ´æ·ÅÔ´Êý¾ÝÆ½¾ùÊý
	float sd[49] = { 0 };  // ±ê×¼Æ«²î 
	float sum[49] = { 0 };
